...
public class deleteCartController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Cart> listRemove = new ArrayList<>();
        for (Cart cart : listCart) {
            //check current productid in list cart equals with pid remove or not
            if (cart.getProduct().getPid() == pid) {
                
                listRemove.add(cart);
            }
        }
        listCart.removeAll(listRemove);
        //if listCart is empty remove listcart on session
        if (listCart.isEmpty()) {
